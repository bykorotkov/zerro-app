                ...
                <div className={classes.Content}>
                    <div className={classes.Block}>
                        ...
                        {user.phone ? (
                            <div className={classes.Line}>
                                <span>Телефон:</span>
                                {` `}
                                <Button href={`tel: ${user.phone}`}>{user.phone}</Button>
                            </div>
                        ) : null}
                        {user.email ? (
                            <div className={classes.Line}>
                                <span>E-mail:</span>
                                {` `}
                                <Button href={`mailto: ${user.email}`}>{user.email}</Button>
                            </div>
                        ) : null}
                        <div className={classes.Line}>
                            <span>Чат:</span>
                            {` `}
                            {user.banned ? user.banned : `Активен`}
                        </div>
                        {user.banReason ? (
                            <div className={classes.Line}>
                    ...
                    <div className={cn(classes.Block, { [classes.Empty]: user.posts && user.posts.length === 0 })}>
                        {user.posts && user.posts.length ? (
                            <div className={classes.Head}>Ваши посты:</div>
                        ) : (
                            <div className={classes.Head}>Постов на данный момент нет. Давайте их создадим!</div>
                        )}
                        {user.posts && user.posts.length ? (
                            <div className={classes.Line}>
                                Ваши посты:
